...
import { DropDownContent, JsonApiResponse, PaginatedResponse, User } from '../../../shared/interfaces';
  ...
  updateUser(user: User) {
    ...
    this.userService.updateUser(user.id, this.formGroup.value).subscribe({
      next: (response) => {
        ...
        const updatedList = currentUsersList.map((u) => u.id === user.id ? updatedUser : u);
