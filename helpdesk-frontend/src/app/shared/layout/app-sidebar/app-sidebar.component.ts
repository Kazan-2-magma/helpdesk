...
`,
  ...
  get filteredNavItems(): NavItem[] {
    ...
    if (userRole === ROLE.ADMIN) {
      ...
      // Show only Dashboard, Tickets, and FAQs for regular users
      return this.navItems.filter((item) =>
        ["Dashboard", "Tickets", "FAQs"].includes(item.name),
      );
    } else if (userRole === ROLE.AGENT) {
      // Show Dashboard and Tickets for agents
      return this.navItems.filter((item) =>
        ["Dashboard", "Tickets"].includes(item.name),
      );
    }
